 ...
 */
var branchStatementBuilder = require('./branch-builder');
...
BranchProcessor.prototype = {
    buildBranchRecordingFunction: function (lineNo, conditionNo) {
        return branchStatementBuilder.buildBranchRecordingFunction(this.fileName, this.functionId++, lineNo, conditionNo);
    },
    ...
    processNode: function (node, parent) {
        ...
        var conditionArrayDeclaration = branchStatementBuilder
            .buildLineAndConditionInitialisation(this.fileName, line, conditionId,
            node.loc.start.column, source.length, source);
        ...
        if (node === parent.test) {
            ...
            throw new Error(util.substitute('Couldn\'t insert wrapper for parent {parent}, node {node} ' +
                'file: {file}, line: {line} ', {
                parent: parentType,
                node: node.type,
                file: this.fileName,
