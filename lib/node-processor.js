...
NodeProcessor.prototype = {
    ...
    buildFunctionInstrumentationStatement: function (lineNumber, functionName) {
        return StatementBuilder.buildFunctionInstrumentationStatement(this.functionNumber++, lineNumber, functionName, this.fileName);
    },
    ...
    processNode: function (node, parent) {
        ...
        if (type === 'FunctionExpression' || type === 'FunctionDeclaration') {
            body = node.body;
            if (body) {
                body.body.unshift.apply(body.body, this.buildFunctionInstrumentationStatement(line, node.id && node.id.name || 'anonymous'));
            }
        }
